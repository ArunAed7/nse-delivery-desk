...
def page() -> None:
    ...
    page_header(
        ...
        "Command",
        "Only NSE delivery, volume, trend and disclosed bulk/block. Sidebar filters apply here. Pin opens a name-level thesis with size cap and invalidation.",
    )
    if snapshot.empty:
    ...
    act_n = int((board["CONVICTION"] == "Act").sum()) if "CONVICTION" in board.columns else 0
    watch_n = int((board["CONVICTION"] == "Watch").sum()) if "CONVICTION" in board.columns else 0
    net_s = board["NET_DISCLOSED_CR"] if "NET_DISCLOSED_CR" in board.columns else pd.Series(dtype=float)
    net_flow = float(pd.to_numeric(net_s, errors="coerce").dropna().sum()) if not board.empty else 0
    names_with_flow = int(pd.to_numeric(net_s, errors="coerce").notna().sum()) if not board.empty else 0
    as_of = last_dt.strftime("%d %b %Y") if last_dt else "—"
    gap_n = int(board["PRICE_UNRELIABLE"].fillna(False).sum()) if "PRICE_UNRELIABLE" in board.columns else 0
    ideas = top_ideas(board, n=6)
    roll = ideas if not ideas.empty else board.nlargest(8, "SETUP_QUALITY") if "SETUP_QUALITY" in board.columns else board.head(0)
    ...
    r1[2].metric("Act / Watch", f"{act_n} / {watch_n}", border=True)
    r2 = st.columns(3, gap="small")
    r2[0].metric("Names on mandate", f"{len(board):,}", border=True)
    r2[1].metric("90d bulk/block net", f"₹{net_flow:,.0f} Cr", border=True)
    r2[1].caption(f"{names_with_flow} names with bulk/block. Not FII/DII.")
    r2[2].metric("Regime", market_regime(d.get("history", pd.DataFrame())), border=True)
    if gap_n:
        st.caption(f"{gap_n} names on this board have unreliable 5D moves (cache gaps). They cannot be Act.")

    left, right = st.columns([1.45, 1], gap="large")
    with left:
        st.subheader("Act / high-conviction tape")
        if ideas.empty:
            empty_state("No setups", "Loosen the sidebar mandate, or wait for delivery+volume confirmation.")
        else:
            for start in range(0, min(len(ideas), 6), 3):
                ...
                for col, (_, idea) in zip(cols, chunk.iterrows()):
                    with col:
                        ...
                        st.caption(str(idea.get("CONVICTION") or ""))
                        st.markdown(idea_card_html(idea), unsafe_allow_html=True)
                        if st.button("Open thesis", key=f"pin_{idea['SYMBOL']}", width="stretch"):
    ...
    with right:
        ...
        rot = sector_rotation(board if not board.empty else snapshot, n=8)
        if rot.empty:
            st.caption("No sector 20D moves.")
        else:
            st.plotly_chart(sector_bar_chart(rot), width="stretch")
            st.caption("Median 20-day price change among names on the filtered board.")
